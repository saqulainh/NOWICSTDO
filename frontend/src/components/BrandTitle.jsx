...
export default function BrandTitle({ className = '' }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex font-light tracking-wide text-[3.5rem] sm:text-[5rem] md:text-[6.5rem] leading-[0.85] text-transparent bg-clip-text bg-gradient-to-br from-[#5eeeb5] via-[#34d99a] to-[#22c97d]">
        {studio.map((char, i) => (
      ...
      <motion.div
        className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 rounded-full bg-[#34d99a] relative"
        initial={{ scale: 0, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.8, duration: 0.5, type: 'spring', stiffness: 200, damping: 12 }}
      >
        <motion.div 
          className="absolute inset-0 rounded-full bg-[#34d99a] blur-md opacity-60"
